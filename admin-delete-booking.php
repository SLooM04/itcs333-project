...
$bookings = [];
$message = ''; // This will hold the success message
$noBookingsMessage = ''; // This will store the "no bookings found" message
...
if (isset($_POST['search'])) {
    ...
    $query = "SELECT b.booking_id, b.room_id, b.room_name, b.start_time, b.end_time, b.status, 
                     t.username AS teacher_name, t.id AS teacher_id, s.username AS student_name, s.id AS student_id
              FROM bookings b
    ...
    if (empty($bookings)) {
        $_SESSION['no_bookings_message'] = "No bookings found for the specified room.";  // Set message in session
    } else {
        unset($_SESSION['no_bookings_message']);  // Clear any previous message if bookings are found
    }
...
if (isset($_GET['cancel_booking_id'])) {
    ...
    if ($stmt->execute()) {
        $_SESSION['message'] = "Booking canceled successfully.";  // Set success message in session
    } else {
        $_SESSION['message'] = "Error canceling booking.";  // Error message in session
    }
...
<div class="container">
    ...
    <!-- Display Success Message (Booking canceled successfully) -->
    <?php if (isset($_SESSION['message']) && !isset($_SESSION['no_bookings_message'])) { ?>
        <p class="success-message" id="success-message"><?php echo $_SESSION['message']; ?></p>
        <?php unset($_SESSION['message']); // Remove message after showing ?>
    <?php } ?>

    <!-- Display 'No bookings found' message only after search -->
    <?php if (isset($_SESSION['no_bookings_message'])) { ?>
        <p class="no-bookings-message"><?php echo $_SESSION['no_bookings_message']; ?></p>
        <?php unset($_SESSION['no_bookings_message']); // Remove message after showing ?>
    <?php } ?>
            ...
            <tbody>
                <?php foreach ($bookings as $booking) { ?>
                    <tr>
                        ...
                        <td><?php echo $booking['student_id'] ?: 'N/A'; ?></td>
                        <td><?php echo $booking['teacher_id'] ?: 'N/A'; ?></td>
                        <td><?php echo !empty($booking['student_name']) ? $booking['student_name'] : $booking['teacher_name']; ?></td>
